# from Lib.site-packages.cv2 import load_config_py3
...
test_text = "CAM05 01:14" # 인식 최악의 예제를 주자
...
scaled = cv2.resize(gray, None, fx=3.0, fy=3.0, interpolation=cv2.INTER_LINEAR) # 인식률 개선시키자

# 이진화 : 글자와 배경을 완전히 흑 / 백으로 분리 (OCR이 글자를 명확하게 인식 할 수 있도록)
_, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU) # 이진화(흑/백) 분리 시키자. 이미지 자동 분석하자. 최적값을 분석하자.

# 4단계 이진화 이후 노이즈 제거 -> 모폴로지 연산
# OPEN: 침식(Erode) -> 팽창(Dilate)
kernel = np.ones((2, 2), np.uint8)
cleaned = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel=kernel) # OPEN은 침식, CLOSE는 팽창
...
def ocr_with_conf(image) -> tuple[str, float] : 
    ...
    result_dict = pytesseract.image_to_data(image, lang='eng', config='--psm 7 --oem 3', output_type=pytesseract.Output.DICT)
