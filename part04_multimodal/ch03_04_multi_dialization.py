...
HF_TOKEN = os.getenv("HF_TOKEN")  # Hugging Face 토큰 (pyannote 모델 다운로드용)
...
waveform = torch.from_numpy(data).unsqueeze(0)  # (time,) → (1, time) "1은 MONO"

output = diarization_pipeline(
    {"waveform": waveform, "sample_rate": 44100},
    min_speakers=2,  # 최소 화자 수 (num_speakers 대신 범위 지정)
