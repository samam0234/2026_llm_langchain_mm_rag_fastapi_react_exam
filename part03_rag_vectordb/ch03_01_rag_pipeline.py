...
# LangChain LCEL RAG 파이프라인 (LCEL : 파이프라인 연산자 (|))
...
# .load() : CSV의 각 행(Row)을 LangChain 표준 문서 단위인 Document 객체 리스트로 변환하여 읽어옵니다.
# - page_content: CSV 행의 데이터들이 자연어 텍스트 형태로 병합된 본문
# - metadata: 행 번호(row) 및 지정한 출처 컬럼(timestamp) 등의 부가 정보
...
splitter = CharacterTextSplitter(
    chunk_size=500,       # 청크 하나 단위의 최대 글자수
...
megative propmt : 주어지지 않은 정보에 대한 추론 금지.
...
llm = ChatOpenAI(model="gpt-4o", temperature=0) # temperature는 추론 값. 높아질수록 정밀한 추론을 한다.
