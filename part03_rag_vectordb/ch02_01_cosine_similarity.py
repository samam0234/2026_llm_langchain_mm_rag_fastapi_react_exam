...
# RAG가 어떻게 자연어로 유사도 검색을 할 수 있는지에 대한 동작 방식 예제
...
def cosine_simlarity(a:np.ndarray, b:np.ndarray) -> float :
    """
    코사인 유사도 계산.
    수식: cos(θ) = (A · B) / (|A| × |B|)
...
for log, log_vec in zip(log_texts, log_vecs) :
    simlarity = cosine_simlarity(query_vec, log_vec)
    results.append((simlarity, log))
