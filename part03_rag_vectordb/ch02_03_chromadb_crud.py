...
# 실행 위치에 영향받지 않도록 스크립트 파일 기준 절대 경로로 설정
current_dir = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(current_dir, "chroma_db") # DB 경로
...
collection = chroma_client.get_or_create_collection(
  ...
  metadata={
    "hnsw:space": "cosine" # 텍스트 임베딩에 가장 적합한 거리 측정방식(방향만 보고 크기무시)
  }
)
...
# 이전 실행에서 log_001이 이미 삭제되었거나 데이터가 없는 경우, 매끄러운 다회성 실행을 위해 초기 상태로 자동 복구합니다.
if collection.count() == 0 or "log_001" not in all_existing_data["ids"]:
  print("=== STEP 1: 초기 데이터 저장 (최초 실행 또는 데이터 자동 복구) ===")
  if collection.count() > 0:
      collection.delete(ids=all_existing_data["ids"])
  ...
  for id_, doc, meta in zip(all_data["ids"], all_data["documents"], all_data["metadatas"]):
      resolved_mark = "✅" if meta.get("resolved", False) else "⬜"
...
collection.update(
    ids=["log_001"],
    embeddings=[get_embedding(new_text)],  # 내용이 바뀌었으므로 벡터 재생성 필수
    documents=[new_text],
...
print(f"log_999: {result999['documents'][0]}")
print(f"    resolved={result999['metadatas'][0]['resolved']} (신규 추가)\n")
