# OPEN AI 채팅 모델을 LangChain방식으로 사용할 수 있도록 하는 클래스
from langchain_openai import ChatOpenAI
# 프롬포트를 템플릿으로 관리할 수 있도록 하는 클래스
from langchain_core.prompts import ChatPromptTemplate
# LLM이 반환한 JSON 문자열을 파이썬 딕셔너리를 변환하는 클래스
# StrOutputParser : 문자열을 그대로 문자열로... (보고서, 요약, 글쓰기등)
from langchain_core.output_parsers import JsonOutputParser, StrOutputParser

# [1] 프롬포트 템플릿 만들기
# {detections_text} 부분에 invoke() 할 때 실제 탐지 결과 문자열이 들어가게 된다.
prompt = ChatPromptTemplate.from_messages([
    ("system", "당신은 보안 전문가입니다. 반드시 JSON으ㅗ만 응답하세요."),   # 페르소나주입, 응답형식 결정
    ("human", "탐지 결과:\n{detection_text}\n\n위험도, 이유, 조치사항을 JSON으로 반환하세요.")  # 사용자 실제요청 메시지
])

# [2] 모델 만들기 (사용할 llm 모델 지정), ChatOpenAI open ai api를 랭체인 방식으로 사용할 수 있도록 하는 클래스
# temperature : (0~2의 실수, 0 : 가장 일관적(분석/분류 적합, 채점), 0.3 : 약간 유연(요약, 보고서), 0.7이상 : 창의적)
llm = ChatOpenAI(model="gpt-4o-mini")

# [3] 출력 파서 만들기, LLM이 반환한 JSON 문자열을 파이썬 딕셔너리를 반환하는 클래스
json_parser = JsonOutputParser()

# [4] Chain으로 연결하기
# LCEL(LangChain Expression Language) : 랭체인에서만 사용하는 전용 연산자 '|'
# 데이터라 흐르는 방향을 표시 (왼쪽에서 오른쪽으로 데이터가 흘러간다)
