...
# 이 코드에서는 에러가 날 수 밖에 없는데 문자열에 백틱을 비롯한 json이라는 문자열이 포함 되어 있기 때문이다.
...
json_parser = JsonOutputParser()    # 파이썬 딕셔너리 객체로 변환
result_auto = json_parser.parse(llm_response_markdown)
