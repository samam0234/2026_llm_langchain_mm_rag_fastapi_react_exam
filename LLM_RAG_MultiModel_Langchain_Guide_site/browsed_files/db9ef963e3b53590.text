...
from langchain_core.messages import (
    ...
    HumanMessage,       # 유저가 LLM에게 전달할 프롬포트를 만들 클래스
...
class CCTVOperatorChatbot : 
    ...
    def __init__(self):
        # 대화 이력 저장소 - 프로그램 실행중(현재 시선)에만 유지 (RAM 저장)
    ...
    def _build_messages(self, user_input: str) -> list:
        ...
        return [
            ...
            *self.history.messages,                # 전체 이전 대화 (언팩 : history.mssages:list의 아이템꺼냄)
    ...
    # JsonOutputParser는 랭체인 전용 컴포넌트, 이 예제는 프레임분석을 요청하는 프롬포트와
    # 일반적인 질문하는(자연어 질문) 프롬포트가 섞여 있기 때문에..
    ...
    def analyze_frame(
        ...
        # exchange (사용자 프롬포트 + 그 프롬포트 의 대한 LLM의 응답)의 쌍으로 저장
